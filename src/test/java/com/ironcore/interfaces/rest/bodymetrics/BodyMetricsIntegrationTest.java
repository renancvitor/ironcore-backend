...
import com.ironcore.infrastructure.persistence.bodymetrics.repository.BodyMetricsJpaRepository;
...
@ActiveProfiles("test")
class BodyMetricsIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        bodyMetricsJpaRepository.deleteAll();
    ...
    class IntegratedFlow {
        ...
        void shouldManageUserBodyMetricsThroughAuthenticatedHttpFlow() throws Exception {
            ...
            UpdateBodyMetricsRequest updateRequest = new UpdateBodyMetricsRequest(
                    77.5,
                    180.0,
                    circumferences(),
                    "Medição atualizada."
            ...
            BodyMetricsEntity updatedMetric = bodyMetricsJpaRepository.findById(secondMetricId).orElseThrow();
            ...
            assertThat(bodyMetricsJpaRepository.findById(firstMetricId)).isEmpty();
            assertThat(bodyMetricsJpaRepository.findById(secondMetricId)).isPresent();
        }
    }
    ...
    private Long createUserBodyMetrics(Cookie accessTokenCookie, CreateBodyMetricsRequest request) throws Exception {
        ...
        assertThat(bodyMetricsJpaRepository.findById(id)).isPresent();

        return id;
    }

    private void updateMeasuredAt(Long metricId, LocalDateTime measuredAt) {
        BodyMetricsEntity entity = bodyMetricsJpaRepository.findById(metricId).orElseThrow();
        entity.setMeasuredAt(measuredAt);
        bodyMetricsJpaRepository.saveAndFlush(entity);
    }

    private CreateBodyMetricsRequest createRequest(Double weightKg, String notes) {
        return new CreateBodyMetricsRequest(
                weightKg,
                180.0,
                circumferences(),
                notes
